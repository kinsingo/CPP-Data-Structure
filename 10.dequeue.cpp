...
typedef char element;    //char형을 덱 element의 자료형으로 정의
typedef struct DQNode {   //이중 연결리스트 덱의 노드 구조를 구조체로 정의
	element data;
	struct DQNode* llink;
	struct DQNode* rlink;
} DQNode;

typedef struct {       //덱에서 사용하는 포인터 front와 rear를 구조체로 정의
	DQNode* front, * rear;
} DQueType;

DQueType* createDQue()  //공백 덱을 생성하는 연산
{
	DQueType* DQ;
...
void insertRear(DQueType* DQ, element item)   //덱의 rear 뒤로 삽입하는 연산
{
	DQNode* newNode = (DQNode*)malloc(sizeof(DQNode));
	newNode->data = item;
...
element deleteRear(DQueType* DQ)  //덱의 rear 노드를 삭제하고 반환하는 연산
{
	DQNode* old = DQ->rear;
	element item;
	if (isEmpty(DQ)) return 0;
...
int removeRear(DQueType* DQ)    //덱의 rear 노드를 삭제하는 연산
{
	DQNode* old = DQ->rear;
	if (isEmpty(DQ)) return 0;
...
void printDQ(DQueType* DQ)   //덱의 front 노드부터 rear 노드까지 출력하는 연산
{
